...
import excelFormat from '../assets/format.png';
...
function CreateToken() {
  ...
  const airdropToUserWallet = async (ownerPubkey, tokensToAirdrop) => {
    try {
      ...
      const transactionSignature =
        await createAssociatedAccountFromMintKeyAndMint(
          connection,
          provider,
      ...
      toast.error(
        `${err ? err.message : 'Ohh no! Exception occured. Please check'}`
      );
      console.log(err);
      return {
        error: `${
  ...
  const denominationHandler = (e) => {
    ...
    try {
      ...
      if (parsed >= 0 && parsed < 10) setDenomination(parsed);
      else alert('Enter valid range 1-9');
    } catch (err) {
      setDenomination('');
  ...
  const initiateTheAirdrop = async () => {
    ...
    const distributionMessage = `https://explorer.solana.com/address/${mintKey}/largest${
    ...
    for (let i = 0; i < rawSheet.length; i++) {
      ...
      if (solAirdrop) {
        result = await transferCustomToken(
          provider,
          connection,
          row.tokensToAirdrop,
          provider.publicKey,
      ...
      row.status =
        result && result.status ? 'Successful' : `Failure: ${result.error}`;
      row.loading = false;
  ...
  return (
    <div>
      ...
      <section class="section-lg seo">
        <div className="flex justify-start loggedInView content-center w-5/6 m-auto">
          <div className="inputView flex flex-1 flex-col justify-center border border-indigo-600">
            <div className="inputWrapper">
              <input
                type="checkbox"
                id="airdrop"
                defaultChecked={solAirdrop}
            ...
            <div className="inputWrapper">
              ...
              <input
                ...
                value={mintKey}
              />
              {mintKey && (mintKey.length <= 42 || mintKey.length >= 45) && (
                <span style={{ color: 'red' }}>
                  Please enter valid mint key
                </span>
              )}
            </div>
            <div className="inputWrapper">
            ...
            <div className="inputWrapper fileWrapper">
              ...
              {fileErrorMessage && fileErrorMessage.length && (
                <span
                  style={{
                    color: 'red',
                    marginLeft: '100px',
              ...
              )}
              {
                <div className="fileName flex-col">
                  {!fileName && (
                    <img src={excel} alt="" style={{ width: '200px' }} />
                  )}
                  {fileName && (
                    <div>
                      {' '}
                      <div className="fileNameRow">{fileName}</div>
          ...
          <div className="flex-3 flex-col flex-grow resultView border border-yellow-400 justify-center ">
            {updatedSheet && updatedSheet.length > 0 && (
              <div>
                <div className="exportButton">
                  ...
                  >
                    <a href={distributionMessage} target="_blank">
                      Check token distribution
                    </a>
                  </div>
                  <button onClick={exportToCSV}>
                    <img src={excel} alt="" style={{ width: '40px' }} /> Export{' '}
                  </button>
                </div>
            ...
            {(!updatedSheet || updatedSheet.length < 1) && (
              <div className="excelFormat">
                <img src={leftArrow} alt="" className="arrow" />
                <h2>Steps to initiate the Airdrop</h2>
                <ul style={{ textAlign: 'left' }}>
                  <ol>
                    1. In case of Custom Token Transfer, Please paste the{' '}
                    <strong>Mint Key</strong> of the token associated with the
                    logged in wallet.
                  </ol>
                  <ol>
                    2. Please mention <strong>Denomination in Lamports</strong>{' '}
                    of the token. Solana max supports 1-9
                  </ol>
                  <ol>
                    3. Please upload the <strong>excel</strong> in below format
                    only.
                    <table className="able-auto border-separate border-2 border-green-900">
                      <tr>
                      ...
                      <tr>
                        ...
                        <td className="border-2 border-green-600">15</td>
                      </tr>
                      <tr>
                        <td className="border-2 border-green-600">
                          4zM2qwPv3DTLjJLhz2UQF2hg1G5XcJwr7hgxjCdxtdtB
                        </td>
                        <td className="border-2 border-green-600">32</td>
                      </tr>
                    </table>
                  </ol>
                  <ol>
                    4. Click on <strong>Airdrop Token</strong> to initiate the
                    airdrop process.
                  </ol>
                </ul>
              </div>
